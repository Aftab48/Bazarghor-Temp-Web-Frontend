...
const MapplsPlayground = () => {
  ...
  const handleAutosuggest = async (e) => {
    e.preventDefault();
    if (!autosuggestQuery.trim()) return;
    setLoadingAction('autosuggest');
    setStatus({ type: '', message: '' });
    try {
      const res = await api.mappls.autosuggest({ input: autosuggestQuery.trim() });
      setResult(res.data || null);
      setStatus({ type: 'success', message: 'Autosuggest results loaded.' });
    } catch (error) {
      setStatus({
        type: 'error',
        message: error.response?.data?.message || 'Failed to fetch autosuggest results.',
      });
    } finally {
      setLoadingAction('');
    }
  ...
  const handleGeocode = async (e) => {
    ...
    setStatus({ type: '', message: '' });
    try {
      const res = await api.mappls.geocode({ address: geocodeQuery.trim() });
      setResult(res.data || null);
  ...
  const handleReverseGeocode = async (e) => {
    ...
    setStatus({ type: '', message: '' });
    try {
      const res = await api.mappls.reverseGeocode({
  ...
  return (
    <Layout>
      <div style={{ maxWidth: '800px', margin: '0 auto' }}>
        <Card title="🗺️ Mappls Playground">
          <p style={{ color: '#4a5568', marginTop: 0 }}>
            Hit the proxy endpoints under <code>/api/mappls</code> without leaving the dashboard.
          </p>
          ...
          <section style={{ marginBottom: '2rem' }}>
            <h3 style={{ marginBottom: '0.5rem', color: '#2d3748' }}>Places Autosuggest</h3>
            <form onSubmit={handleAutosuggest}>
              <Input
                ...
                placeholder="e.g., coffee shop in Kolkata"
                required
              />
              <Button type="submit" disabled={loadingAction === 'autosuggest'}>
                {loadingAction === 'autosuggest' ? 'Searching...' : 'Search'}
              </Button>
            </form>
          </section>
...
export default MapplsPlayground;
