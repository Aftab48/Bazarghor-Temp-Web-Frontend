...
const DeliveryPartnerLogin = () => {
  ...
  const handleSendOTP = async (e) => {
    ...
    try {
      const response = await api.deliveryPartner.sendOTP({ mobNo: formData.mobNo });
  ...
  const handleVerifyOTP = async (e) => {
    ...
    try {
      const response = await api.deliveryPartner.verifyOTP({
  ...
  const handleResendOTP = async () => {
    ...
    try {
      const response = await api.deliveryPartner.resendOTP({ mobNo: formData.mobNo });
  ...
  return (
    <Layout>
      <div style={{ maxWidth: '500px', margin: '0 auto' }}>
        <Card title="🚚 Delivery Partner Login">
          {step === 'send-otp' ? (
          ...
          <div style={{
            ...
            <div style={{ fontFamily: 'monospace', marginTop: '0.5rem' }}>
              POST /api/delivery-partner/login/send-otp<br />
              POST /api/delivery-partner/login/verify<br />
              POST /api/delivery-partner/login/resend
            </div>
          </div>

          <div style={{ textAlign: 'center', marginTop: '1rem' }}>
            <span style={{ color: '#718096' }}>Don't have an account? </span>
            <a href="/delivery-partner/register" style={{ color: '#667eea', fontWeight: '600' }}>
              Register here
            </a>
          </div>
        </Card>
